 ...
 */
public class MockQuestionSetServiceFactory implements Factory<QuestionSetService> {
  ...
  public QuestionSetService provide() {
    ...
    Mockito.when(mockedService.findQuestionSets()).thenAnswer(new Answer<List<QuestionSet>>() {
      public List<QuestionSet> answer(InvocationOnMock invocation)
          throws Throwable {
    ...
    Mockito.when(mockedService.findQuestionSetByQuestionSet(QUESTIONSETNAME)).thenAnswer(new Answer<QuestionSet>() {
      public QuestionSet answer(InvocationOnMock invocation)
          throws Throwable {
      return new QuestionSet(QUESTIONSET1_NAME, QUESTIONSET1_DESC);
      }
    });

    Mockito.when(mockedService.findQuestionSetByQuestionSet(UNCHECKED_EXCEPTION)).thenThrow(new IllegalArgumentException(OUR_EXCEPTION_MESSAGE));

    Mockito.when(mockedService.findQuestionSetByQuestionSet(NON_EXISTING_QUESTIONSETNAME)).thenAnswer(new Answer<QuestionSet>() {
      public QuestionSet answer(InvocationOnMock invocation)
              throws Throwable {
        return null;
      }
  ...
  public void dispose(QuestionSetService t) {
  }
}
