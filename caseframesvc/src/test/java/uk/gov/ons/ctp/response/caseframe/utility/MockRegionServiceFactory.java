 ...
 */
public class MockRegionServiceFactory implements Factory<RegionService> {
  ...
  public RegionService provide() {
    ...
    Mockito.when(mockedService.findById(REGION_WITH_CODE_CHECKED_EXCEPTION)).thenThrow(new IllegalArgumentException(OUR_EXCEPTION_MESSAGE));

    Mockito.when(mockedService.findAll()).thenAnswer(new Answer<List<Region>>() {
      public List<Region> answer(InvocationOnMock invocation)
              throws Throwable {
    ...
    Mockito.when(mockedService.findAllLadsByRegionid(REGION_WITH_CODE_REG123)).thenAnswer(new Answer<List<LocalAuthority>>() {
      public List<LocalAuthority> answer(InvocationOnMock invocation)
              throws Throwable {
        String regionCode = (String)invocation.getArguments()[0];
    ...
    Mockito.when(mockedService.findAllLadsByRegionid(REGION_WITH_CODE_204)).thenAnswer(new Answer<List<LocalAuthority>>() {
      public List<LocalAuthority> answer(InvocationOnMock invocation)
              throws Throwable {
  ...
  public void dispose(RegionService t) {}
}