...
public class CaseServiceTest {
    ...
    /**
     * Note that the Integer values below are linked to the order in which cases
     * appear in the array defined at CaseServiceTest.Case.json = casePK
     */
    ...
    /**
     * All of these tests require the mocked repos to respond with predictable data
     * loaded from test fixture json files.
     *
    ...
    public void testCreateCaseEventWithTargetCase() throws CTPException {
        ...
        CaseEvent caseEvent = new CaseEvent(1, NON_EXISTING_PARENT_CASE_FK, CASEEVENT_DESCRIPTION, CASEEVENT_CREATEDBY,
                currentTime, CategoryDTO.CategoryName.CASE_CREATED, CASEEVENT_SUBCATEGORY, metadata);
    ...
    public void testCreateCaseEventAgainstNonExistentCase() throws CTPException {
        ...
        metadata.put("partyId", UUID.randomUUID().toString());
        CaseEvent caseEvent = new CaseEvent(1, NON_EXISTING_PARENT_CASE_FK, CASEEVENT_DESCRIPTION, CASEEVENT_CREATEDBY,
                currentTime, CategoryDTO.CategoryName.COMPLETED_BY_PHONE, CASEEVENT_SUBCATEGORY, metadata);
    ...
    public void testCreateNonActionableEventAgainstInactionableCase() throws Exception {
        ...
        CaseEvent caseEvent = fabricateEvent(CategoryDTO.CategoryName.NO_LONGER_REQUIRED,
        ...
        verify(actionSvcClient, times(0)).postAction(any(String.class), any(UUID.class), any(String.class));
    }

    /**
     * We create a CaseEvent with category CASE_CREATED on an initial BRES case (the
    ...
    public void testEventCaseCreated() throws Exception {
        ...
        verify(caseSvcStateTransitionManager, never()).transition(any(CaseState.class), any(CaseDTO.CaseEvent.class));
        verify(notificationPublisher, never()).sendNotification(any(CaseNotificationDTO.class));
        verify(actionSvcClient, never()).postAction(any(String.class), any(UUID.class), any(String.class));
    }
    ...
    /**
     * We create a CaseEvent with category ACTION_COMPLETED on an ACTIONABLE BRES
    ...
    public void testEventActionCompleted() throws Exception {
        ...
        CaseEvent caseEvent = fabricateEvent(CategoryDTO.CategoryName.ACTION_COMPLETED,
    ...
    public void testCaseGroupStatusIsTransitioned() throws Exception {
        ...
        CaseEvent caseEvent1 = fabricateEvent(CategoryDTO.CategoryName.SUCCESSFUL_RESPONSE_UPLOAD,
                ACTIONABLE_BUSINESS_UNIT_CASE_FK);
    ...
    public void testCaseGroupStatusNotUpdated() throws Exception {
        ...
        CaseEvent caseEvent1 = fabricateEvent(CategoryDTO.CategoryName.SECURE_MESSAGE_SENT,
                ACTIONABLE_BUSINESS_UNIT_CASE_FK);
    ...
    /**
     * We create a CaseEvent with category ACTION_CANCELLATION_COMPLETED on an
     * ACTIONABLE BRES case (the one created for a respondent BI, accountant
     * replying on behalf of Tesco for instance)
     *
     * @throws Exception if fabricateEvent does
     */
    @Test
    ...
    /**
     * We create a CaseEvent with category ACCESS_CODE_AUTHENTICATION_ATTEMPT on an
     * ACTIONABLE BRES case (the one created for a business unit B, Tesco for
     * instance)
     *
     * @throws Exception if fabricateEvent does
     */
    ...
    /**
     * We create a CaseEvent with category RESPONDENT_ENROLED on an ACTIONABLE BRES
     * case (the one created for a business unit B, Tesco for instance)
     *
     * @throws Exception if fabricateEvent does
     */
    ...
    public void testEventRespondentEnrolled() throws Exception {
        ...
        verify(caseSvcStateTransitionManager, times(1)).transition(any(CaseState.class), any(CaseDTO.CaseEvent.class));
    ...
    /**
     * Tests that with multiple survey ids then the limit per survey matches
     * maxCasesPerSurvey and that the later cases in the list are rejected This
     * suffices because SpringBoot implements the findByDateTime desc , so we only
     * need validate the later ones are removed
     *
     * @throws Exception
     */
    @Test
    public void testFindCasesByPartyIdLimitedPerSurveyLimitsCases() throws Exception {
        ...
        List<Case> results = caseService.findCasesByPartyIdLimitedPerSurvey(partyId, false, maxCasesPerSurvey);
    ...
    /**
     * We create a CaseEvent with category COLLECTION_INSTRUMENT_DOWNLOADED on an
     * ACTIONABLE BRES case (the one created for a respondent BI, accountant
     * replying on behalf of Tesco for instance)
     *
     * @throws Exception if fabricateEvent does
    ...
    /**
     * We create a CaseEvent with category UNSUCCESSFUL_RESPONSE_UPLOAD on an
     * ACTIONABLE BRES case (the one created for a respondent BI, accountant
     * replying on behalf of Tesco for instance)
     *
     * @throws Exception if fabricateEvent does
    ...
    /**
     * We create a CaseEvent with category OFFLINE_RESPONSE_PROCESSED on an
     * ACTIONABLE BRES case (the one created for a respondent BI, accountant
     * replying on behalf of Tesco for instance)
     *
     * @throws Exception if fabricateEvent does
    ...
    /**
     * A SUCCESSFUL_RESPONSE_UPLOAD event transitions an actionable BI case to
     * INACTIONABLE, and all associated BI cases in the case group. The action
     * service is notified of the transition of all BI Cases to stop them receiving
     * communications.
     *
     * @throws Exception if fabricateEvent does
     */
    ...
    public void testEventSuccessfulResponseUploaded() throws Exception {
        ...
        ArgumentCaptor<Case> argument = ArgumentCaptor.forClass(Case.class);
        verify(caseRepo, times(1)).saveAndFlush(argument.capture());
        verify(caseSvcStateTransitionManager, times(1)).transition(any(CaseState.class), any(CaseDTO.CaseEvent.class)); // action
                                                                                                                        // service
                                                                                                                        // should
                                                                                                                        // be
                                                                                                                        // told
    ...
    public void testEventSuccessfulNoActiveEnrolmentCaseEvent() throws Exception {
        ...
        CaseEvent caseEvent = fabricateEvent(CategoryName.NO_ACTIVE_ENROLMENTS, ACTIONABLE_BUSINESS_UNIT_CASE_FK);
        caseService.createCaseEvent(caseEvent);
    ...
    /**
     * caseService.createCaseEvent will be called with invalid state transitions but
     * suppress the exception. This is expected behaviour. This code smells but
     * keeping as is.
     */
    @Test
    public void testGivenCaseGroupChangeIsInvalidWhenTransitionFailGracefully() throws Exception {
        ...
        doThrow(new CTPException(CTPException.Fault.BAD_REQUEST)).when(caseGroupService)
    ...
    public void testGenerateEnrolmentCodeCaseEvent() throws CTPException {
        ...
        when(internetAccessCodeSvcClient.generateIACs(1)).thenReturn(Collections.singletonList(IAC_FOR_TEST));

        CaseEvent caseEvent = fabricateEvent(CategoryDTO.CategoryName.GENERATE_ENROLMENT_CODE,
                ACTIONABLE_BUSINESS_UNIT_CASE_FK);
        caseService.createCaseEvent(caseEvent);
    ...
    public void testCreateInitialCaseWithSampleUnitChildren() throws Exception {
        ...
        SampleUnitChildren sampleUnitChildren = new SampleUnitChildren(
                new ArrayList<>(Collections.singletonList(sampleUnit)));
        ...
        List<CollectionExerciseDTO> collectionExercises = FixtureHelper
                .loadClassFixtures(CollectionExerciseDTO[].class);
        when(collectionExerciseSvcClient.getCollectionExercise(any())).thenReturn(collectionExercises.get(0));
        ...
        assertEquals(collectionExerciseId, capturedCaseGroup.get(0).getCollectionExerciseId().toString());
    ...
    public void testFindByCaseFKAndCategoryCanMapStringsToEnumValues() throws CTPException {
        ...
        // Then
        verify(caseEventRepo).findByCaseFKAndCategoryInOrderByCreatedDateTimeDesc(eq(123), argumentCaptor.capture());
        assertTrue(argumentCaptor.getValue().contains(EQ_LAUNCH));
        assertThat(argumentCaptor.getValue().size()).isEqualTo(1);
    }

    @Test(expected = CTPException.class)
    public void testFindByCaseFKAndCategoryBlowsUpWhenStringDoesNotMapToEnumValue() throws CTPException {
        // Given
    ...
    private void mockupCaseEventRepo() {
        when(caseEventRepo.save(any(CaseEvent.class))).thenAnswer(invocation -> invocation.getArguments()[0]);
    }
    ...
    private void mockupCollectionExerciseServiceClient() throws Exception {
        List<CollectionExerciseDTO> collectionExerciseDTOs = FixtureHelper
                .loadClassFixtures(CollectionExerciseDTO[].class);
    ...
    private void mockStateTransitions() throws CTPException {
        ...
        when(caseSvcStateTransitionManager.transition(CaseState.ACTIONABLE, CaseDTO.CaseEvent.DEACTIVATED))
                .thenReturn(CaseState.INACTIONABLE);
        when(caseSvcStateTransitionManager.transition(CaseState.ACTIONABLE, CaseDTO.CaseEvent.ACTIONPLAN_CHANGED))
