DROP FUNCTION IF EXISTS casesvc.generate_report_chasing_report();
...
BEGIN
  ...
  v_rows := 0;
  v_contents := '';
  v_contents := 'Sample Unit Ref, Sample Unit Type, Case ID, Authentication Attempt No Account Created, Account Created No Enrolment , Collection Instrument Downloaded No Successful Response Upload ';
  ...
  PERFORM casesvc.logmessage(
      p_messagetext := 'GENERATING RESPONSE CHASING REPORT COMPLETED ROWS WRIITEN = ' || v_rows
      , p_jobid := 0
      , p_messagelevel := 'INFO'
      , p_functionname := 'casesvc.generate_response_chasing_report');
  ...
  WHEN OTHERS
    THEN
      PERFORM casesvc.logmessage(
          p_messagetext := 'GENERATING RESPONSE CHASING REPORT EXCEPTION TRIGGERED SQLERRM: ' ||
