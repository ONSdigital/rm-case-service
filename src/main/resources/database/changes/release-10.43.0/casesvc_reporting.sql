...
CREATE OR REPLACE FUNCTION casesvc.logmessage(p_messagetext  text DEFAULT NULL :: text,
                                              p_jobid        numeric DEFAULT NULL :: numeric,
                                              p_messagelevel text DEFAULT NULL :: text,
...
BEGIN
  ...
  FOR r_dataline IN (SELECT
                     ...
                     FROM
                       (SELECT
                          ...
                          MAX(CASE WHEN ce.categoryFK = 'ACCESS_CODE_AUTHENTICATION_ATTEMPT'
                            THEN 1
                              ELSE 0 END) access_code_authentication_attempt_ind
                          --(B)  -- count distinct event
                          ,
                          ...
                          MAX(CASE WHEN ce.categoryFK = 'COLLECTION_INSTRUMENT_DOWNLOADED'
                            THEN 1
                              ELSE 0 END) collection_instrument_downloaded_ind
                          --(BI) -- count distinct event
                          ,
                          MAX(CASE WHEN ce.categoryFK = 'SUCCESSFUL_RESPONSE_UPLOAD'
                            THEN 1
                              ELSE 0 END) successful_response_upload_ind
                          --(BI) -- count distinct event
                          ...
                          SUM(CASE WHEN ce.categoryFK = 'ACTION_CANCELLATION_COMPLETED'
                            THEN 1
                              ELSE 0 END) action_cancellation_completed
                          --(B,BI) -- count all events
                          ,
                          SUM(CASE WHEN ce.categoryFK = 'ACTION_CANCELLATION_CREATED'
                            THEN 1
                              ELSE 0 END) action_cancellation_created
                          --(B,BI) -- count all events
                          ,
                          ...
                          MAX(CASE WHEN ce.categoryFK = 'OFFLINE_RESPONSE_PROCESSED'
                            THEN 1
                              ELSE 0 END) offline_response_processed_ind
                          --(BI)	 -- count distinct event
                          ,
                     ...
                     ORDER BY events.sampleunitref
                       ...
                       , events.casePK) LOOP
    v_contents :=
    v_contents || chr(10) || r_dataline.sampleunitref || ',' || r_dataline.sampleunittype || ','
    || r_dataline.casePK || ','
  ...
  THEN
    ...
    INSERT INTO casesvc.report (id, reportPK, reporttypeFK, contents, createddatetime) VALUES
      (gen_random_uuid(), nextval('casesvc.reportPKseq'), 'CASE_EVENTS', v_contents,
       CURRENT_TIMESTAMP);
  END IF;

  PERFORM casesvc.logmessage(
      p_messagetext := 'GENERATING CASE EVENTS REPORT COMPLETED ROWS WRIITEN = ' || v_rows
      , p_jobid := 0
      , p_messagelevel := 'INFO'
      , p_functionname := 'casesvc.generate_case_events_report');
  ...
  WHEN OTHERS
    THEN
      PERFORM casesvc.logmessage(
          p_messagetext := 'GENERATING CASE EVENTS REPORT EXCEPTION TRIGGERED SQLERRM: ' ||
...
BEGIN
  ...
  FOR r_dataline IN (SELECT
                       ...
                       CASE WHEN events.access_code_authentication_attempt_ind = 1 AND
                         ...
                         THEN 1
                       ELSE 0 END                  authentication_attempt_no_account_created  --(B)
                       ,
                       (events.respondent_account_created -
                        events.respondent_enroled) account_created_no_enrolment  --(B)
                       ,
                     ...
                     FROM
                       (SELECT
                          ...
                          SUM(CASE WHEN ce.categoryFK = 'RESPONDENT_ACCOUNT_CREATED'
                            THEN 1
                              ELSE 0 END) respondent_account_created
                          --(B)  -- count all events
                          ,
                          SUM(CASE WHEN ce.categoryFK = 'RESPONDENT_ENROLED'
                            THEN 1
                        ...
                        GROUP BY cg.sampleunitref
                          , c.sampleunittype
                          , c.casePK) events
                     WHERE (events.access_code_authentication_attempt_ind = 1 AND
                            events.respondent_account_created = 0)
  ...
  THEN
    -- Insert the data into the report table
    INSERT INTO casesvc.report (id, reportPK, reporttypeFK, contents, createddatetime) VALUES
      (gen_random_uuid(), nextval('casesvc.reportPKseq'), 'RESPONSE_CHASING', v_contents,
       CURRENT_TIMESTAMP);
  END IF;

  PERFORM casesvc.logmessage(
      p_messagetext := 'GENERATING REPORT CHASING REPORT COMPLETED ROWS WRIITEN = ' || v_rows
      , p_jobid := 0
      , p_messagelevel := 'INFO'
      , p_functionname := 'casesvc.generate_report_chasing_report');
  ...
  WHEN OTHERS
    THEN
      PERFORM casesvc.logmessage(
          p_messagetext := 'GENERATING REPORT CHASING REPORT EXCEPTION TRIGGERED SQLERRM: ' ||
