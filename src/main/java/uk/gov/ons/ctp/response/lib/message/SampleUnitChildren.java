//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference
// Implementation, v2.2.8-b130911.1802
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a>
// Any modifications to this file will be lost upon recompilation of the source schema.
// Generated on: 2020.04.30 at 05:48:38 PM BST
//

package uk.gov.ons.ctp.response.lib.message;

import jakarta.xml.bind.annotation.XmlAccessType;
import jakarta.xml.bind.annotation.XmlAccessorType;
import jakarta.xml.bind.annotation.XmlType;
import java.util.ArrayList;
import java.util.List;

/**
 * Java class for SampleUnitChildren complex type.
 *
 * <p>The following schema fragment specifies the expected content contained within this class.
 *
 * <pre>
 * &lt;complexType name="SampleUnitChildren">
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="sampleUnitChild" type="{http://ons.gov.uk/ctp/response/casesvc/message/sampleunitnotification}SampleUnit" maxOccurs="unbounded" minOccurs="0"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(
    name = "SampleUnitChildren",
    propOrder = {"sampleUnitChild"})
public class SampleUnitChildren {

  protected List<SampleUnit> sampleUnitChild;

  /**
   * Gets the value of the sampleUnitChild property.
   *
   * <p>This accessor method returns a reference to the live list, not a snapshot. Therefore any
   * modification you make to the returned list will be present inside the JAXB object. This is why
   * there is not a <CODE>set</CODE> method for the sampleUnitChild property.
   *
   * <p>For example, to add a new item, do as follows:
   *
   * <pre>
   *    getSampleUnitChild().add(newItem);
   * </pre>
   *
   * <p>Objects of the following type(s) are allowed in the list {@link SampleUnit }
   */
  public List<SampleUnit> getSampleUnitChild() {
    if (sampleUnitChild == null) {
      sampleUnitChild = new ArrayList<SampleUnit>();
    }
    return this.sampleUnitChild;
  }
}
