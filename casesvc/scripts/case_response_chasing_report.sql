...
, events.caseFK                                                                                                
, (events.respondent_account_created - events.respondent_enroled) rc_account_no_enrolment  --(B) 
, CASE WHEN events.access_code_authentication_attempt_ind = 1  AND events.respondent_account_created 	 = 0 THEN 1 ELSE 0 END  rc_authentication_but_no_account  --(B)  
, CASE WHEN events.collection_instrument_downloaded_ind   = 1  AND events.successful_response_upload_ind = 0 THEN 1 ELSE 0 END  rc_download_no_upload --(BI)
FROM 
(SELECT 
    cg.sampleunitref
  , c.sampleunittype 
