-- Function: casesvc.insert_helpline_report_into_report()

-- DROP FUNCTION casesvc.insert_helpline_report_into_report();

CREATE OR REPLACE FUNCTION casesvc.insert_helpline_report_into_report()
  RETURNS boolean AS
$BODY$
DECLARE
...
PERFORM casesvc.logmessage(p_messagetext := 'insert_helpline_report_into_report started '
                               ,p_jobid := 0   
                               ,p_messagelevel := 'INFO'
                               ,p_functionname := 'casesvc.insert_helpline_report_into_report');
...
INSERT INTO casesvc.report (reporttype,  contents, createddatetime ) VALUES(v_reporttype,  v_contents_text, v_createddatetime );
   ...
   PERFORM casesvc.logmessage(p_messagetext := 'insert_helpline_report_into_report completed '
                               ,p_jobid := 0   
                               ,p_messagelevel := 'INFO'
                               ,p_functionname := 'casesvc.insert_helpline_report_into_report');
  RETURN TRUE;
  ...
  EXCEPTION
  WHEN OTHERS THEN
   PERFORM casesvc.logmessage(p_messagetext := 'insert_helpline_report_into_report' || SQLERRM
                               ,p_jobid := 0   
                               ,p_messagelevel := 'INFO'
                               ,p_functionname := 'casesvc.insert_helpline_report_into_report');
  RETURN FALSE;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION casesvc.insert_helpline_report_into_report()
  OWNER TO casesvc;
